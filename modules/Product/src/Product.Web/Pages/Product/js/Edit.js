...
var createTierPriceModal = new abp.ModalManager(abp.appPath + 'Product/CreateTierPriceModal');
var editTierPriceModal = new abp.ModalManager(abp.appPath + 'Product/EditTierPriceModal');

$('#createTierPrice').click(function (e) {
    e.preventDefault()
    createTierPriceModal.open({ id: productId })
});

createTierPriceModal.onResult(function () {
    location.reload()
});

editTierPriceModal.onResult(function () {
    location.reload()
});
...
function getTierPrices() {
    var list = []
    var tierPriceList = $('#TierPrice').val()

    if (tierPriceList != null && tierPriceList != '') {
        list = JSON.parse(tierPriceList)
    }
    return list
}

function initializeTierPrice() {
    let tierPriceTable;

    if (!$.fn.DataTable.isDataTable('#tierPriceTable')) {
                                    ...
                                    [
                                        {
                                            ...
                                            action: function (data) {
                                                editTierPriceModal.open({ id: data.record.Id, productId: productId })
                                            }
                                        },
                                        {
                                            text: l('Delete'),
                                            ...
                                            confirmMessage: function (data) {
                                                return l(
                                                    'TierPriceDeletionConfirmationMessage',
                                                    data.record.CustomerGroup
                                                );
                                            },
                                            action: function (data) {
                                                product.controllers.product
                                                    .deleteTierPrice(data.record.id)
                                                    .then(function () {
        ...
        tierPriceTable.draw(false);
    }   
}

document.addEventListener('DOMContentLoaded', function () {
    const previewCarousel = document.getElementById('image-preview');
    const uploadedItems = previewCarousel.querySelectorAll('.carousel-item');
    const activeItem = previewCarousel.querySelector('.carousel-item.active');

    if (uploadedItems.length > 0) {
        const activeIndex = Array.from(uploadedItems).indexOf(activeItem);
        updateThumbnailCounter(activeIndex + 1, uploadedItems.length);
    }

    setupThumbnailCounter(); // Also setup the event listener for moving between slides
});
